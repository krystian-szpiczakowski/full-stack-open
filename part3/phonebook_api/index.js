...
const validateNewPersonRequest = (person) => {
    ...
    const duplicatedPerson = persons.find(p => p.name.toLowerCase() === person.name.toLowerCase())
    if (duplicatedPerson) {
        errors.push({
...
app.get("/info", (request, response) => {
    ...
    response.send(`<p>Phonebook has info for ${phonebookEntriesNumber} people</p><p>${currentDate}</p>`)
})
